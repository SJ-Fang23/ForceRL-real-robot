...
# read the masks (npz)
mask = np.load('mask_drawer.npz')

drawer_mask = mask["mask"]
print(drawer_mask.shape)
...
def callback(ros_data):
    ...
    # apply the mask
    cv_image = cv_image * drawer_mask
    np.save('depth_image_drawer.npy', cv_image)
    ...
    cv2.imwrite('depth_image_masked_drawer.png', color_image)
